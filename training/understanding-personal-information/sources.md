<!-- Generated from course.json by training/lib/build-course.mjs. Edit the JSON and regenerate. -->

# Research behind Personal Information in Plain Words

Research reviewed: 2026-09-21. The lessons, fictional packet, hints, answer keys, and timing are original proposals. No learner trial has been performed for this course.

## S1

**[Protecting Personal Information: A Guide for Business](https://www.ftc.gov/business-guidance/resources/protecting-personal-information-guide-business)**

Publisher: US Federal Trade Commission

Published or updated: October 2016 (month only)

Reviewed: 2026-09-21

**Supports:** Five principles: take stock, scale down, lock it, pitch it, plan ahead. Says not to keep sensitive data without a legitimate business need and, in its words, 'don't even collect it'; limit employee access to what a role requires; lists places data ends up, including home computers and mobile devices; have an incident response plan with a coordinator and notification steps.

**Our application:** The minimum-collection paragraph, the 'Old' sheet retention flag, and the approved-locations point.

**Limits:** Written for US businesses under US law in 2016. It does not define personal information for other jurisdictions or set notification duties outside the US.

**Revisit when:** If the FTC revises the guide or the firm's operating jurisdictions change.

## S2

**[What is personal information: a guide](https://ico.org.uk/for-organisations/uk-gdpr-guidance-and-resources/personal-information-what-is-it/what-is-personal-information-a-guide/)**

Publisher: UK Information Commissioner's Office

Published or updated: Updated 19 May 2023

Reviewed: 2026-09-21

**Supports:** Personal data is information relating to an identified or identifiable individual. Identifiers include names, identification numbers, location data, and online identifiers; a combination of identifiers may be needed, and indirect identification through information reasonably likely to be combined still counts. Pseudonymised data is still personal data. Special category data attracts stricter rules.

**Our application:** The lesson's definition of personal information and its treatment of combinations, partial identifiers, and the Notes column.

**Limits:** UK GDPR guidance; definitions and duties differ elsewhere. The page gives no worked examples, so the packet's examples are mine. The ICO's separate small-organizations page on this topic returned not found on the review date, so this general guide is cited instead.

**Revisit when:** If the ICO updates the guide or UK data protection law changes.

## S3

**[SP 800-122: Guide to Protecting the Confidentiality of Personally Identifiable Information (PII)](https://csrc.nist.gov/pubs/sp/800/122/final)**

Publisher: US National Institute of Standards and Technology

Published or updated: April 2010 (Final)

Reviewed: 2026-09-21

**Supports:** Defines PII as information that can distinguish or trace an individual or is linked or linkable to one, with examples including name, identification numbers, email address, phone numbers, and financial, medical, and employment information; partial identifiers are often PII. Impact factors include quantity of records, data field sensitivity, and access from devices or systems outside the organization's direct control. Recommends the 'minimum necessary' principle and regular review and destruction of PII with no current purpose.

**Our application:** The list of what counts, the 'a list is more sensitive' paragraph, the location paragraph, and the 'Old' sheet question.

**Limits:** Written for US federal agencies in 2010 and not revised since. Its impact-level scheme is a federal framework, not a rule for private firms. The landing page shows the abstract; the cited sections are in the linked PDF.

**Revisit when:** If NIST withdraws or supersedes SP 800-122, or if the NIST Privacy Framework changes the definitions.

## S4

**[Protecting bulk personal data](https://www.ncsc.gov.uk/collection/protecting-bulk-personal-data)**

Publisher: UK National Cyber Security Centre

Published or updated: Published 25 September 2016, last reviewed 21 November 2018

Reviewed: 2026-09-21

**Supports:** Large stores of personal data are 'very tempting targets for attackers of all kinds'. Fifteen good-practice measures grouped by knowing what you are protecting, restricting access to those with a real need, and sound design and operation.

**Our application:** The 'a list weighs more than one record' paragraph and the checklist item on limiting the extract to what the task needs.

**Limits:** Written for organizations running large data systems, not for one spreadsheet, and last reviewed in 2018. It does not define personal data or address legal duties.

**Revisit when:** If the NCSC replaces the collection or the firm starts holding materially larger data sets.

## S5

**[Personal data breaches: a guide](https://ico.org.uk/for-organisations/report-a-breach/personal-data-breach/personal-data-breaches-a-guide/)**

Publisher: UK Information Commissioner's Office

Published or updated: Updated 20 August 2025

Reviewed: 2026-09-21

**Supports:** A breach includes sending personal data to the wrong recipient, lost or stolen devices, and unauthorized access. Organizations must assess risk, record every breach, notify the ICO within 72 hours where required, and tell individuals where risk is high. Recommends allocating breach handling to a dedicated person or team and making sure staff know how to escalate.

**Our application:** The 'report the same day to the named person, do not investigate alone' guidance and the D3 exposure rule.

**Limits:** UK GDPR duties. Thresholds and deadlines differ in other jurisdictions; the lesson deliberately leaves that to the firm's privacy lead.

**Revisit when:** If the ICO revises the guide or the firm's jurisdictions change.

[Back to the course](README.md)
