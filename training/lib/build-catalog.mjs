...
${entries.length} modules across three tracks: ${entries.filter((entry) => entry.format === 'bite').length} short modules of one or two lessons and ${entries.filter((entry) => entry.format !== 'bite').length} courses of three or more. Every module works on paper or in a document, uses a fictional packet, and needs no tool account. Suggested times are starting estimates.
