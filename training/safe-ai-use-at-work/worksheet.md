...
# Using AI Without Leaking: worksheet
...
## Lesson 2: Instructions hiding in the material
...
**Task:** One packet document is unsafe: it carries text written to steer an assistant. Quote the line and its aim, then state what the assistant should have been allowed to do and what stays with Mara.
...
## Lesson 3: Check before it counts

**Outcome:** Produce a corrected draft with each correction traced to the packet.

**Task:** Read D3 against D1. Trace every figure, name, date, quote, and status claim to a line in D1, or note that you cannot. Then write a corrected briefing for Ines, each fix marked with the line that settles it.

**Deliverable:** A corrected briefing with traced fixes, a removals list, and a line on disclosure.

- [ ] Correct the status and price; remove the unsourced comparison.
- [ ] Give the escrow quote to its real author; fix the call date.
- [ ] Drop the send-to-Rowan claim and the bank and identity details; Ines needs neither.
