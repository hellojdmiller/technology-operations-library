...
const client = new Client({ name: 'tol-catalog-demo-client', version: '0.1.0' });
...
try {
  ...
  const page = await client.callTool({ name: 'tol_list_services', arguments: { criticality: 'tier-1', limit: 2, response_format: 'json' } });
  ...
  const detail = await client.callTool({ name: 'tol_get_service', arguments: { service_id: 'svc-fund-operations' } });
